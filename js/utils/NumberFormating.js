...
function formatShort(decimal, precision = 2, small) {
    ...
    else if (decimal.gte(1000)) return exponentialFormat(decimal, precision)
    else if (decimal.gte(100)) return commaFormat(decimal, 0)
    else if (decimal.gte(0.0001) || !small) return regularFormat(decimal, precision)
    else if (decimal.eq(0)) return (0).toFixed(precision)
...
function formatTime(s) {
    ...
    else if (s < 1e9) return formatWhole(Math.floor(s / 31536000)) + "y " + formatWhole(Math.floor(s / 86400) % 365) + "d " + formatWhole(Math.floor(s / 3600) % 24) + "h " + formatWhole(Math.floor(s / 60) % 60) + "m " + format(s % 60) + "s"
